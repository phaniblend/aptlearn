#!/usr/bin/env python3
"""
Generate 5 complete sample lessons following APT LEARN LESSON SPECIFICATION v1.0
...
def save_lesson(tech_dir, lesson):
    """Save lesson to appropriate directory."""
    ...
    print(f"✓ Created: {file_path.name} ({len(lesson['flow'])} steps, {sum(len(str(step.get('mentorSays', ''))) for step in lesson['flow'])} chars)")
...
# Due to response length limits, I'll create the remaining 4 lessons in a continuation file
# For now, let me save this first lesson and create a script structure

if __name__ == '__main__':
    # Save React lesson
    save_lesson(LESSON_GEN_DIR / "react", react_hello_world)
    
    print("\n[INFO] Lesson 1 of 5 created. Remaining 4 lessons will be created in next part.")
